        ...
        client.on('interactionCreate',  (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<:M_x:950441844544794654> You already selected something, this Selection is now disabled!`, ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<:M_x:950441844544794654> You are not allowed to do that! Only: <@${cmduser.id}>`, ephermal : true});
          }
        });
      }
