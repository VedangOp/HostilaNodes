    ...
    try {
      ...
      async function first_layer() {
        ...
        let menumsg = await message.reply({
          embeds: [MenuEmbed],
          components: [row1, row2, row3, row4, new MessageActionRow().addComponents(new MessageButton().setStyle("LINK").setURL("https://youtu.be/QGESDc31d4U").setLabel("Tutorial Video").setEmoji("950445233118773248"))]
        })
        ...
        collector.on('collect', menu => {
          if (menu?.user.id === cmduser.id) {
            ...
            content: `<:M_x:950441844544794654> You are not allowed to do that! Only: <@${cmduser.id}>`,
            ephemeral: true
          });
        });
        ...
        collector.on('end', collected => {
          menumsg.edit({
            ...
            components: [],
            content: `<a:Yes:950441315823411243> **Selected: \`${collected && collected.first() && collected.first().values ? collected.first().values[0] : "Nothing"}\`**`
          })
        });
      }
      async function second_layer() {
        ...
        let menumsg = await message.reply({
          embeds: [MenuEmbed],
          components: [new MessageActionRow().addComponents(Selection), new MessageActionRow().addComponents(new MessageButton().setStyle("LINK").setURL("https://youtu.be/QGESDc31d4U").setLabel("Tutorial Video").setEmoji("950445233118773248"))]
        })
        //Create the collector
        const collector = menumsg.createMessageComponentCollector({
          filter: i => i?.isSelectMenu() && i?.message.author.id == client.user.id && i?.user,
        ...
        switch (optionhandletype) {
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<:M_x:950441844544794654> **You need to add at least 1 Open-Apply-Option**")
            }
            ...
            if (collected && collected.first().content) {
              ...
              if (collected2 && collected2.first().mentions.channels.size > 0) {
                ...
                return message.reply("<:M_x:950441844544794654> **You did not ping a valid Channel!**")
              }
            } else {
              return message.reply("<:M_x:950441844544794654> **You did not enter a Valid Message in Time! CANCELLED!**")
            }
          }
          break;
          case "Add Apply Option": {
            let data = theDB.get(message.guild.id, pre+".data");
            if (data.length >= 25) {
              return message.reply("<:M_x:950441844544794654> **You reached the limit of 25 different Options!** Remove another Option first!")
            }
            ...
            if (collected && collected.first().content) {
              if (!collected.first().content.includes("++")) return message.reply("<:M_x:950441844544794654> **Invalid Usage! Please mind the Usage and check the Example**")
              let value = collected.first().content.split("++")[0].trim().substring(0, 25);
              let index = data.findIndex(v => v.value == value);
              if(index >= 0) {
                  return message.reply("<:M_x:950441844544794654> **Options can't have the SAME VALUE!** There is already an Option with that Value!");
              }
              ...
              collector.on('collect', menu => {
                if (menu?.user.id === cmduser.id) {
                  ...
                  if(index >= 0) {
                      return message.reply("<:M_x:950441844544794654> **Options can't start the Same Apply System!** There is already an Option with that Application System!");
                  }
                  ...
                  if(!apply_for_here.has(message.guild.id) || !apply_for_here.has(message.guild.id, applypre) || !apply_for_here.has(message.guild.id, applypre+".QUESTIONS") || apply_for_here.get(message.guild.id, applypre+".QUESTIONS").length < 1) 
                    return message.reply(`<:M_x:950441844544794654> **The ${applySystemExecution}. Apply System is not setupped / has no Questions, create it first with: \`${prefix}setup-apply\`**`)
          ...
          let data = theDB.get(message.guild.id, pre+".data");
          if (!data || data.length < 1) {
            return message.reply("<:M_x:950441844544794654> **There are no Open-Apply-Options to remove**")
          }
          let embed = new MessageEmbed()
            .setColor(es.color)
            .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
          ...
          collector.on('end', collected => {
            menumsg.edit({
              embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
              components: [],
              content: `<a:Yes:950441315823411243> **Selected: \`${collected.first().values[0]}\`**`
            })
          });
        }
        break;
