      ...
      c.on("voiceStateUpdate", (oldState, newState) => {
        if (!oldState.channelId && newState.channelId) {
          ...
          return send_log(c,
            ...
            "https://cdn.discordapp.com/attachments/849047781276647425/869529604296159282/950792370306314321.gif"
            )
        }
        if (oldState.channelId && !newState.channelId) {
          if (
...
async function send_log(c, guild, color, title, description, thumb, fieldt, fieldv, fieldt2, fieldv2) {
  try {
    ...
    const LogEmbed = new Discord.MessageEmbed()
      ...
      .setFooter(c.getFooter(guild?.name + "Hostila Nodes | https://hostilanodes.xyz/", guild?.iconURL({
        format: "png"
      })))
    if(fieldt && fieldv){
      if(fieldv.trim() !== ">>>") {
